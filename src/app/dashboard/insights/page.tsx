...
export default function InsightsPage() {
    ...
    const {
        ...
        const allLogs = [
            ...fuelLogs.map((log) => ({ date: log.date, cost: log.total_cost, type: "fuel" as const, odometer: log.odometer })),
            ...maintenanceLogs.map((log) => ({ date: log.date, cost: log.cost || 0, type: "maintenance" as const, odometer: undefined })),
            ...customLogs.map((log) => ({ date: log.date, cost: log.cost || 0, type: "custom" as const, odometer: undefined })),
        ].sort((left, right) => new Date(left.date).getTime() - new Date(right.date).getTime());

        let cost = 0;
        let minOdometer = selectedVehicle.baseline_odometer;
        let maxOdometer = selectedVehicle.baseline_odometer;
        ...
        for (const log of allLogs) {
            ...
            if (typeof log.odometer === "number") {
                maxOdometer = Math.max(maxOdometer, log.odometer);
                minOdometer = Math.min(minOdometer, log.odometer);
            }
        ...
        const firstLogDate = allLogs[0] ? new Date(allLogs[0].date) : null;
        const lastLogDate = allLogs[allLogs.length - 1] ? new Date(allLogs[allLogs.length - 1].date) : null;
        const trackingDays = firstLogDate && lastLogDate
            ? Math.max(1, Math.floor((lastLogDate.getTime() - firstLogDate.getTime()) / (1000 * 60 * 60 * 24)))
            : 0;
        const distance = Math.max(0, maxOdometer - minOdometer);
