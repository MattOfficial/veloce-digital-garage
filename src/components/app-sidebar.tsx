...
const navigation = [
    ...
    { name: ui.sidebar.items.maintenance, href: "/dashboard/maintenance", icon: Wrench },
    { name: ui.sidebar.items.runningCosts, href: "/dashboard/insights", icon: BarChart2 },
];
...
export function AppSidebar() {
    ...
    const displayName = profile.displayName || profile.email?.split('@')[0] || brand.app.shortName;
    ...
    return (
        <Sidebar collapsible="icon">
            ...
            <SidebarContent>
                <SidebarGroup>
                    <SidebarGroupLabel className="group-data-[collapsible=icon]:hidden">{ui.sidebar.sectionTitle}</SidebarGroupLabel>
                    <SidebarGroupContent>
                        <SidebarMenu>
                            {navigation.map((item) => (
                ...
                <div className="flex flex-col gap-2 p-4 group-data-[collapsible=icon]:p-0 group-data-[collapsible=icon]:py-4 border-t border-border/50">
                    <Link href="/dashboard/profile" className="flex items-center gap-3 w-full hover:bg-secondary/50 p-2 rounded-full transition-all duration-300 active:scale-95 hover:scale-[1.02] cursor-pointer mb-2 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:p-0">
                        ...
                        <div className="flex flex-col overflow-hidden text-left group-data-[collapsible=icon]:hidden">
                            ...
                            <span className="text-xs text-muted-foreground truncate leading-tight">{ui.common.profile.viewProfile}</span>
                        </div>
                    </Link>
                    <form action={logout} className="w-full flex justify-center">
                        ...
                        >
                            ...
                            <span className="group-data-[collapsible=icon]:hidden">{ui.common.profile.logOut}</span>
                        </SidebarMenuButton>
                    </form>
                </div>
            </SidebarFooter>
