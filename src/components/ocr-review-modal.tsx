...
export function OcrReviewModal({
  ...
  const updateOcrField = <K extends keyof OcrExtractedData>(
    field: K,
    value: OcrExtractedData[K],
  ) => {
    setOcrData((currentData) =>
      currentData ? { ...currentData, [field]: value } : currentData,
    );
  };
  ...
  const updateLineItem = (
    ...
    value: OcrLineItem["service"] | OcrLineItem["cost"],
  ) => {
    setOcrData((currentData) => {
      if (!currentData) {
        return currentData;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        const lineItemsText = ocrData.line_items
          .map((lineItem) => `- ${lineItem.service}: $${lineItem.cost}`)
          .join("\n");
        ...
        const maintenanceEntry = {
          ...
          service_type: ocrData.provider
            ? ui.maintenance.ocr.serviceAt(ocrData.provider)
            : ui.maintenance.ocr.generalMaintenance,
        ...
        const { error: insertError } = await supabase
          ...
          .insert(maintenanceEntry);
        if (insertError) throw insertError;

        // Also record the document in our new documents table
        if (filePath) {
          const docName =
            filePath.split("/").pop() || ui.maintenance.ocr.defaultDocumentName;
          ...
          <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div>
              <p className="font-semibold text-foreground">
                {ui.maintenance.ocr.processingTitle}
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                {ui.maintenance.ocr.processingDescription}
              </p>
            </div>
          </div>
        ) : error ? (
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateOcrField("total_cost", e.target.value)
                    }
            ...
            <div className="space-y-3 pt-4 border-t border-white/10">
              <Label className="text-xs text-muted-foreground uppercase flex items-center justify-between">
                ...
                <span className="text-[10px] bg-primary/20 text-primary px-2 py-0.5 rounded-full">
                  {ocrData.line_items?.length || 0}{" "}
                  {ui.maintenance.ocr.foundSuffix}
                </span>
              </Label>
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                {ocrData.line_items.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex gap-2 items-center bg-black/20 p-2 rounded-lg border border-white/5"
                  >
                    <Input
                      value={item.service}
                      onChange={(e) =>
                        updateLineItem(idx, "service", e.target.value)
                      }
                    ...
                    <div className="relative w-24 shrink-0">
                      ...
                      <Input
                        ...
                        value={item.cost}
                        onChange={(e) =>
                          updateLineItem(idx, "cost", e.target.value)
                        }
                ...
                {(!ocrData.line_items || ocrData.line_items.length === 0) && (
                  <p className="text-sm text-muted-foreground italic text-center py-4">
                    {ui.maintenance.ocr.noLineItems}
                  </p>
                )}
              </div>
            </div>
              ...
              >
                {isPending ? (
                  ...
                  <>
                    ...
                    {ui.maintenance.ocr.verifyAndSave}
                  </>
                )}
              </Button>
            </DialogFooter>
